...
| [2026-09-07 (21)](2026-09-07-21-tach-rac-detector-va-hota-hop-gt.md) | M4 + M6 | **Tách 217 ID rác** (`diagnose_junk_ids.py`, đối chiếu thẳng chú thích WildTrack): **73.5% khối lượng là hộp detector báo nhầm** (`match_rate` trung vị 0.00), 26.5% là id-switch của NvDCF — cả hai NGOÀI `src/mct`. Phần "người thật bị loại" trượt vì độ thuần khiết (track lẫn người), không nới bảng nhãn được. Chấm `onnx_gtbox`: hộp GT quy ra HOTA 16.21 → **25.18 (+55%)**, nhưng 2/3 là DetA (bỏ 15407 FP); phần liên kết là **AssA +33%**. Rác 217 → 59 khi bỏ FP; vỡ/gộp không đổi |
| [2026-09-10 (22)](2026-09-10-22-mat-ut-hpc-va-cong-cu-confidence.md) | hạ tầng + M6 | **Mất `ut-hpc`**: test chuyển về venv 3.10 (`uv`), WildTrack sinh lại trên `vast-gpu` T4 ($0.23; sửa `unzip_wildtrack` hỏng vì bản vá CVE-2024-0450 của `zipfile`). **Nhiễu giữa hai lần chạy pipeline cùng cấu hình ~0.3–0.9 HOTA** — cỡ hiệu ứng, nên so cấu hình pipeline cần ≥3 lần chạy. Quét `pre-cluster-threshold` thật (n=3): 0.25 → 0.40 giảm Global ID 407 → 318 và DetA, tăng IDF1 20.1 → 22.1 (t≈3), nhưng **HOTA 15.70 ± 0.46 → 16.00 ± 0.40 không tách khỏi nhiễu** — đánh đổi gần hoà, không phải cải tiến; 0.55 mất DetA. Confidence AUC TP/FP 0.750; lọc fixture ĐÁNH GIÁ THẤP lợi ích (tracker thật dựng track sạch hơn). Không đổi config thật — chốt ở M6 |
